export function AudioBuffer2WavFile(buffer, startTime, endTime){
  const url = writeWav(
    AudioBuffer2WavData(buffer, startTime, endTime),
    buffer.sampleRate
  );
  return url;
}

function AudioBuffer2WavData(buffer, startTime, endTime){
...
export function writeWav(samples, sampleRate){
  ...
  writeWavHead(view, samples.length, sampleRate);
  writeWavData(view, samples);
...
function writeWavHead(view, sample_length, sampleRate){
...
function writeWavData(view, samples){
